...
const createEleeyeModule = require(path.join(workerDir, 'eleeye.js'));
...
createEleeyeModule({
  ...
  mod.ccall('init_eleeye_engine', null, [], []);
  ...
  mod.ccall('execute_ucci_command', null, ['string'], [`position fen ${startFen}`]);

  console.log('[测试 3] 调用 execute_ucci_command("go depth 6") 进行 6 层 Alpha-Beta 真实算力搜索...');
  const startTime = Date.now();
  mod.ccall('execute_ucci_command', null, ['string'], ['go depth 6']);
