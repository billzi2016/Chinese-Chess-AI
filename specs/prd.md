# 产品需求文档 (PRD): Web 象棋对弈系统 (Eleeye-WASM)

## 1. 项目概述

* **项目名称**：Chinese-Chess-AI（开源轻量 Web 象棋）
* **定位**：零后端开销、纯前端运行的现代化中国象棋对弈应用。
* **核心亮点**：
  * **零服务器计算**：象眼引擎（Eleeye）通过 WebAssembly（WASM）纯本地运行。
  * **主线程零卡顿**：引擎放入 Web Worker，单核并行计算，UI 保持 60fps 响应。
  * **极致轻量**：全局静态资源包小于 1MB，加载秒开，支持部署于 GitHub Pages。
...
| **AI 对弈** | 5 秒定时思考 | 玩家落子后，向 Eleeye Worker 发送 `go movetime 5000`。引擎在 **5秒内** 返回 `bestmove`。 | **P0** |
| **规则判定** | 胜负与合法性 | 基于 `xiangqi.js` 自动拒绝违规走法，并在出现困毙、胜负或长将时弹出结果提示。 | **P0** |
